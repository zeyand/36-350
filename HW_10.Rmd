...
```{r}
readLines("https://raw.githubusercontent.com/zeyand/36-350/master/README.md")
```
...
library(devtools)
source_url("https://raw.githubusercontent.com/zeyand/36-350/master/hello_world.R")
```
...
```{r, error = TRUE}
f = function(x) { 
  if (typeof(x) != "character") stop("x must be a character.")
  tolower(x) 
}
f(1)
```
...
```{r, error = TRUE}
f = function(x) { 
  message("This function tranform characters to lower case.")
  if (typeof(x) != "character") stop("x must be a character.")
  tolower(x) 
}
f(7)
suppressWarnings(message(f("A")))
```
...
```{r, error = TRUE}
f = function(x) { 
  tryCatch(sqrt(x), error = function(c) "Error: input is not a number.", warning = function(c) "Warning: input is a negative number")
...
```{r, error = TRUE}
f = function(x) { 
  tryCatch(sqrt(x), error = function(c) "Error: input is not a number.", warning = function(c) "Warning: input is a negative number", finally = print(x))
...
```{r, error = TRUE}
library(testthat)
test_that(desc = "Test for improper input (numeric)", expect_null(word.similarity(1, 2)))
test_that(desc = "Test for expected output length (1)",     expect_length(word.similarity("abc","acb"), 1))
test_that(desc = "Test that output is double",                  expect_type(word.similarity("abc", "acb"),"double"))
test_that(desc = "Test that upper-case letters work",            expect_error(word.similarity("A", "B")))
test_that(desc = "Test for overlaps in the words", expect_equal(word.similarity("aaa","zzz"), 0))
test_that(desc = "Test for inputs that are NULL", expect_null(word.similarity(NULL, NULL)))
```
...
word.similarity = function(word.1,word.2)
{
  if (is.character(word.1) == FALSE) return(NULL)
  if (is.character(word.2) == FALSE) return(NULL)
  word.1.integers = sapply(unlist(strsplit(word.1,"")),function(x){which(letters==x)})
  word.2.integers = sapply(unlist(strsplit(word.2,"")),function(x){which(letters==x)})
  suppressWarnings(wilcox.test(x=word.1.integers,y=word.2.integers)$statistic[[1]])
}
